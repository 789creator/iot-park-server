...
BEGIN_MESSAGE_MAP(CRealLoadPictureDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_OPEN_STROBE, &CRealLoadPictureDlg::OnBnClickedButtonOpenStrobe)
	//}}AFX_MSG_MAP		
...
void CRealLoadPictureDlg::OnButtonLogin() 
{
	...
	int error = 0;
	char* szUserName = m_strUserName.GetBuffer(0);
 	LLONG lLoginID = CLIENT_LoginEx2(szIp, (WORD)m_nPort, m_strUserName.GetBuffer(0), 
                                    m_strPasswd.GetBuffer(0), EM_LOGIN_SPEC_CAP_TCP, 
	...
	//Login ok
	m_lLoginID = lLoginID;
	m_nChannel = netDevInfo.nChanNum;
...
void CRealLoadPictureDlg::OnBnClickedButtonOpenStrobe()
{
	if (m_lLoginID <= 0)
	{
		MessageBox(ConvertString("Please login first!"),ConvertString("Prompt"));
	...
	stuOpenStrobe.nChannelId = nSel;	
	BOOL bRet = CLIENT_ControlDeviceEx(m_lLoginID, DH_CTRL_OPEN_STROBE, &stuOpenStrobe);
	if (!bRet)
	{		
		MessageBox(ConvertString("Open Strobe fail"), ConvertString("Prompt"));
	}
}
...
BOOL CRealLoadPictureDlg::UpdateLicensePlateNumber(const DWORD dwEventType, const void* pEventInfo)
{
	...
	case EVENT_IVS_TRAFFIC_PARKINGSPACEPARKING:
		{
			DEV_EVENT_TRAFFIC_PARKINGSPACEPARKING_INFO* pInfo = (DEV_EVENT_TRAFFIC_PARKINGSPACEPARKING_INFO*)pEventInfo;	
			strLicensePlate = MakeLP(pInfo->stTrafficCar.szPlateNumber);	
		}
		break;
	case EVENT_IVS_TRAFFIC_PARKINGSPACENOPARKING:
		{			
			DEV_EVENT_TRAFFIC_PARKINGSPACENOPARKING_INFO* pInfo = (DEV_EVENT_TRAFFIC_PARKINGSPACENOPARKING_INFO*)pEventInfo;		
			strLicensePlate = MakeLP(pInfo->stTrafficCar.szPlateNumber);	
		}
		break;
