...
void CActiveLoginDemoDlg::login(DH_DEVICE_INFO* pInfo)
{
    ...
    if (NULL !=pInfo)
    {
        if (1 == pInfo->nLoginStatus || pInfo->nLoginTimes >0)
        {
			...
			{
				loginID = CLIENT_LoginEx2(pInfo->szIP, pInfo->nPort, pInfo->szUseName,pInfo->szPassWord,EM_LOGIN_SPEC_CAP_TCP ,NULL,&deviceInfo, &nError);
			...
			else if (AUTO == pInfo->nLoginType)
			{
				...
				loginID = CLIENT_LoginEx2(pInfo->szIP, pInfo->nPort, pInfo->szUseName,pInfo->szPassWord,EM_LOGIN_SPEC_CAP_SERVER_CONN ,(void*)szSerial,&deviceInfo, &nError);
			}
			if (loginID != 0)
            {
                m_csDeviceInfos.Lock();
				pInfo->lLoginID = loginID;
				pInfo->nChannelNum = deviceInfo.nChanNum;
